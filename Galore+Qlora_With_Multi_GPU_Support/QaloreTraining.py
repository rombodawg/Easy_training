...
def save_training_state(checkpoint_dir: str, step: int, epoch: int,
    ...
    state = {
        ...
        'optimizer_state': accelerator.gather(optimizer.state_dict()), # Gather optimizer state
        'scheduler_state': accelerator.gather(scheduler.state_dict())  # Gather scheduler state
    }
...
def main() -> None:
    ...
    if torch.cuda.is_available():
        device_map = {"": accelerator.local_process_index} # Use accelerator's local process index, removed condition for device_map="auto"
    else:
        device_map = "cpu"
