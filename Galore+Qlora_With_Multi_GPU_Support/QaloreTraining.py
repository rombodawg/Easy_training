...
def main():
    ...
    # Limit to a maximum of 8 GPUs
    if num_gpus > 8:
        num_gpus = 8

    if torch.cuda.is_available():
        if num_gpus > 1:
            # When using 4-bit quantization, we want to use the "auto" device map so that
            # the model is sharded across GPUs rather than fully replicated (which can OOM).
    ...
    # Only wrap with DataParallel if we're not using device_map "auto" (which shards the model)
    ...
    for epoch in range(start_epoch, config["num_epochs"]):
        ...
        for step, batch in progress_bar:
            ...
            # With DataParallel, the input on cuda:0 is automatically scattered to all GPUs;
            # when using "auto", the model is already distributed.
